...
注册进会话视图环（`conversation.view`）的**第三个视图 tab**（id `source-control`，order 20，与对话/轨迹并列）；tab 活跃时主体内左右分栏（左 SCM 分组/提交区、右 diff 查看器）；切走即卸载；选中工作区持久化到 localStorage。

- tab 头部：工作区切换下拉（默认 `recentWorkspaceId`）。
