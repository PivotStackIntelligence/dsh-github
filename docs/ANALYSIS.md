...
GitLens 将仓库导航、分支导航和浏览器链接作为相互独立的功能。插件沿用功能划分的思路，但把改动、提交、分支、同步和浏览器链接收敛进会话视图环里的 Source Control tab（与对话/轨迹并列，主结构内左右分栏，绑定当前会话工作区）：改动/提交区 + 懒加载的 COMMITS、BRANCHES、REMOTES、TAGS、STASHES 区段。
...
src/client/view.tsx    会话视图环里的 Source Control tab：注册 conversation.view 槽位、绑定当前会话 cwd（byId[sessionId].cwd）、随 tab 挂载/卸载
src/client/panel.tsx   tab 主体内的 SCM 面板：左分组/提交区、右 diff 查看器、懒加载区段与操作状态
...
| --- | --- |
| Source Control view | 会话视图环里的 Source Control tab（主结构内左右分栏，绑定当前会话工作区） |
...
- 真实 Web Harness 重启后的 Source Control 视图 tab（与对话/轨迹并列、挂载/卸载与轮询启停、无 cwd 提示）、diff 查看器、各区段与 GIT OUTPUT、操作面板和浏览器 console/page error 检查。
