...
- **视图 tab** — 在会话视图环中注册第三个 "Source Control" tab（与对话/轨迹并列）；tab 活跃时主体内左右分栏（左 SCM 分组/提交区、右 diff 查看器），无遮罩不遮挡，切回对话即恢复。选中的工作区跨刷新记住。
- **仓库头部** — 工作区切换下拉（默认 recentWorkspaceId）、仓库（workspace）名、当前分支、ahead/behind 数量、GitHub 链接和刷新按钮。
...
- **自动刷新** — 挂载即刷新，tab 活跃期间每 3 秒轮询一次（切走即卸载、轮询随之停止），窗口 focus / visibilitychange 时立即刷新；懒加载区段（commits、branches、remotes、tags、stashes）首次展开时加载，之后随每次 status 刷新。
...
重新构建插件后需要重启 Web Harness。会话视图环中会出现第三个 "Source Control" tab（与对话/轨迹并列），打开后可用头部下拉切换工作区。
