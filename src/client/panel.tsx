...
type Operation = 'stage' | 'unstage' | 'stageAll' | 'unstageAll' | 'commit' | 'push' | 'fetch' | 'pull' | 'sync' | 'checkoutBranch' | 'createBranch' | null
...
function badge(file: GitFileChange): string { return file.kind === 'untracked' ? 'U' : file.kind === 'deleted' ? 'D' : file.kind === 'added' ? 'A' : file.kind === 'renamed' ? 'R' : 'M' }
...
export function GithubChangesPanel({ path, title, actions, t, onClose }: {
  ...
  const runStatusAction = (name: Exclude<Operation, null>, action: () => Promise<RemoteResult<GitStatus>>, clearMessage = false): void => {
    ...
    }).catch(reason => setError(errorMessage(reason))).finally(() => setOperation(null))
  }
  ...
  const changeRow = (file: GitFileChange, mode: GitDiffMode) => <div className={`dsh-github-change-row ${selected?.path === file.path && selected.mode === mode ? 'selected' : ''}`} key={`${mode}:${file.path}`}>
    <button type="button" className="dsh-github-change-main" onClick={() => setSelected({ path: file.path, mode })}>
      <span className={`kind kind-${file.kind}`}>{badge(file)}</span><span title={file.path}>{file.path}</span>
    </button>
  ...
  const group = (mode: GitDiffMode, files: GitFileChange[], expanded: boolean, setExpanded: (value: boolean) => void) => <section className="dsh-github-change-group">
    <div className="dsh-github-change-group-header">
      <button type="button" className="dsh-github-group-toggle" aria-expanded={expanded} onClick={() => setExpanded(!expanded)}><span>{expanded ? '⌄' : '›'}</span>{mode === 'staged' ? t('panel.stagedChanges') : t('panel.changes')} <b>{files.length}</b></button>
    ...
    <section className="dsh-github-panel">
      <header className="dsh-github-panel-header">
        ...
        <div className="dsh-github-panel-actions">
          <button type="button" onClick={tab === 'changes' ? refresh : refreshOverview} aria-label={t('panel.refresh')} title={t('panel.refresh')}>↻</button>
      ...
      {!loading && status && tab === 'changes' ? <div className="dsh-github-source-layout">
        <aside className="dsh-github-source-sidebar">
          ...
          {group('staged', staged, stagedExpanded, setStagedExpanded)}
          {group('working', working, workingExpanded, setWorkingExpanded)}
      ...
      {!loading && status && tab === 'repository' ? <div className="dsh-github-overview">
        ...
        {overview ? <>
          <section><h2>{t('panel.branches')} <span>{overview.branches.length}</span></h2>
            ...
            <div className="dsh-github-card-list">{overview.branches.map(branch => <button type="button" className={`dsh-github-branch-card ${branch.current ? 'current' : ''}`} disabled={operation !== null || branch.current} key={`${branch.remote ? 'remote' : 'local'}:${branch.name}`} onClick={() => runStatusAction('checkoutBranch', () => actions.checkoutBranch(path, branch.name, branch.remote))}><strong>{branch.name}</strong><small>{branch.current ? t('panel.currentBranch') : branch.remote ? t('panel.remoteBranch') : branch.upstream ?? t('panel.localBranch')}</small></button>)}</div>
          </section>
