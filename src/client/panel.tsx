...
import type { GitBranch, GitDiff, GitDiffMode, GitFileChange, GitRepositoryOverview, GitStatus } from '../types.ts'
      ...
      {!loading && status && tab === 'repository' ? <div id="dsh-github-tabpanel-repository" role="tabpanel" aria-labelledby="dsh-github-tab-repository" className="dsh-github-overview">
        ...
        {overview ? <>
          <section><h2>{t('panel.branches')} <span>{overview.branches.length}</span></h2>
            ...
            <div className="dsh-github-branch-groups">{([
              { kind: 'local', title: 'panel.localBranches' as const, branches: overview.branches.filter(branch => !branch.remote) },
              { kind: 'remote', title: 'panel.remoteBranches' as const, branches: overview.branches.filter(branch => branch.remote && pullRequestNumber(branch) === null) },
              { kind: 'pullRequest', title: 'panel.pullRequestBranches' as const, branches: overview.branches.filter(branch => pullRequestNumber(branch) !== null) },
            ]).map(group => {
              if (group.branches.length === 0) return null
              return <section className="dsh-github-branch-group" key={group.kind}><h3>{t(group.title)}</h3><div className="dsh-github-card-list">{group.branches.map(branch => {
                const pr = pullRequestNumber(branch)
                return <div className={`dsh-github-branch-card ${branch.current ? 'current' : ''}`} key={`${branch.remote ? 'remote' : 'local'}:${branch.name}`}>{pr === null ? <button type="button" className="dsh-github-branch-switch" disabled={operation !== null || branch.current} onClick={() => runStatusAction('checkoutBranch', () => actions.checkoutBranch(path, branch.name, branch.remote))}><strong>{branch.name}</strong><small>{branch.current ? t('panel.currentBranch') : branch.remote ? t('panel.remoteBranch', { name: overview.remoteName ?? t('panel.remote') }) : branch.upstream ?? t('panel.localBranch')}</small></button> : <div className="dsh-github-branch-switch dsh-github-branch-label"><strong>{t('panel.pullRequest', { number: pr })}</strong><small>{branch.name}</small></div>}{branch.branchUrl ? <a className="dsh-github-branch-link" href={branch.branchUrl} target="_blank" rel="noreferrer" aria-label={`${t('panel.openBranch')}: ${branch.name}`}>↗</a> : null}</div>
              })}</div></section>
            })}</div>
          </section>
